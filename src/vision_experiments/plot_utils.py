...
def load_runs(run_path_forward, run_path_backward, optimizers, run_name) :
  ...
  for opt in run_path_backward :
      for strat in ["forward", "backward"] :
          run_path = f"{run_name}/forward/"+run_path_forward[opt] if strat=="forward" else f"{run_name}/backward/"+run_path_backward[opt]
          for k in metrics:
...
def plot_comparison(train_data, optimizers, val_data=None):    
    metrics = ["Loss", "Acc", "F1"]
    opt_colors = {"SGD":"red", "MomentumSGD":"blue", "ClippedSGD":"green", "Adagrad":"orange", "Adam":"purple"}
    if val_data is not None:
        opt_colors = {"basic_convNet" : "red", "resnet18" : "blue", "small_vit" : "green"}
    ...
    for k in range(len(metrics)):
        ...
        for opt in optimizers:
                ...
                if val_data is not None:
                    axes[0].plot(val_data["forward"][opt][metric], label=f'forward {opt} val', linestyle='dashed', alpha=0.6, color = opt_colors[opt] )
                ...
                if val_data is not None:
                    axes[1].plot(val_data["backward"][opt][metric], label=f'backward {opt} val', linestyle='dashed', alpha=0.6, color = opt_colors[opt] )
...
def plot_train_valid(train_data, opt, x_axis,val_data=None):
    ...
    for k in range(len(metrics)):
        ...
        for i in range(len(strats)):
            ...
            axes[k][0].set_title(f'backward vs forward {metric} by {x_axis}')
            axes[k][0].set_xlabel(x_axis)
            axes[k][0].set_ylabel(metric)
            if k == 0:
                axes[k][0].legend()
            axes[k][0].grid(True)
            axes[k][1].plot(train_data[strats[i][0]][opt]["CumTime"],train_data[strats[i][0]][opt][metric], label=f'{strats[i][0] +opt} train', alpha=0.6, color=strats[i][1])
            if val_data is not None:
            ...
            axes[k][1].set_title(f'backward vs forward by Time (s)')
